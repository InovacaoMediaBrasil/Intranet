...
function onUnavailableMonitors () {
  $('.monitor').empty()
  const img = $('<img />')
    .attr('src', 'Images/Monitors/unavailable.png')
    .attr('title', 'Monitor não disponível')
  $('.monitor').append(img)
}
...
function onAvaliableMonitors (data) {
  ...
  if (data.length == 0) {
    $('ul#monitorsList').append(
      "<li class='list-group-item list-group-item-danger text-center'><i class='fa fa-thumbs-down ' aria-hidden='true'></i> Nenhum monitor disponível!</li>"
    )
  }
  const showTime = typeof isMonitors === 'undefined' || !isMonitors
  data.sort(orderByName)
  if (!showTime) {
    data.sort(function (a, b) {
      const ao = a.CurrentStatus == 3 ? 0 : a.CurrentStatus == 1 ? 1 : 2
      const bo = b.CurrentStatus == 3 ? 0 : b.CurrentStatus == 1 ? 1 : 2
  ...
  $.each(data, function (index, item) {
    ...
    if (actual === null) actual = { CurrentStatus: 2 }
    let src
    let text = item.ErrorMessage
    let className
    let icon
    const showNotification =
      (typeof isMonitors !== 'undefined' && isMonitors) ||
    ...
    switch (item.CurrentStatus) {
      ...
      case 2:
        ...
        if (
          (actual.CurrentStatus === 1 || actual.CurrentStaus === 3) &&
          showNotification
        ) {
          notification(
            'success',
            item.Category,
            'Acesso ao sistema/serviço ' +
              item.Name +
              ' normalizado às ' +
              date
          )
      ...
      case 3:
        ...
        if (item.ErrorMessage === '') {
          textNotification =
            'O sistema/serviço ' + item.Name + ' está em estado de alerta!'
    ...
    img.attr(
      ...
      'Sistema ' +
        text +
        " <br /> <span style='font-size:12px;text-decoration:underline;'>Atualizado às " +
        date +
        '</span>'
    )
    ...
    const li =
      ...
      (showTime
        ? "<a href='" +
          item.URL +
          "' target='_blank' style='color:#000 !important;'><i class='fa fa-external-link' aria-hidden='true'></i></a> <span style='font-size:9px;'>Atualizado em " +
          date +
          '</span></li>'
        : '')
    if (
      item.CurrentStatus !== 0 ||
      typeof hidePausedMonitors === 'undefined' ||
      !hidePausedMonitors
    ) {
...
function storeMonitorsInCookie (data) {
  ...
  window.Cookies.set('monitors', temp, {
    expires: 365,
