if (typeof inlineLoaded === 'undefined') var inlineLoaded = false
if (typeof jQuery === 'undefined') var jQuery = null;
...
(function ($, window, document, inlineLoaded, undefined) {
    ...
    $.ajax({
      ...
      beforeSend: function (xhr) {
        ...
        xhr.setRequestHeader(
          'X-INTEGRACAOSERVICEAPI-TOKEN',
          'yMzanJ6ZuBNBSMlH9dtAyaf6JU5LT86x'
        )
      },
  ...
  function loadScriptIf (condition, src, callback, callbackIfLoad) {
    console.groupCollapsed(
      'Should load script ' + src + ' ? ' + (condition ? 'Yes' : 'No')
    )
    ...
    if (condition) {
      loadScript(src, function () {
        if (typeof callbackIfLoad === 'function') callbackIfLoad()
  ...
  function processIp (ip) {
    ...
    if (
      ip.indexOf('192.168.1.') !== -1 &&
      ip !== window.Cookies.get('intranet.lanip')
    ) {
      checkIfIsInIntranet()
    }
    window.Cookies.set('intranet.lanip', ip, {
      expires: 30,
      domain: '.editorainovacao.com.br',
      secure: true,
      sameSite: 'none'
    })
  }
  ...
  function checkIfIsInIntranet () {
    if (window.Cookies.get('intranet.isLocalIntranet')) isLocalIntranet = true
    console.warn(
      'Currently connected through: ' + (isLocalIntranet ? 'LAN' : 'WAN')
    )
    return
    // Fix the SSL certificate of 192.168.1.235
    const img = new Image()
    $(img).on('load', function () {
      window.Cookies.set('intranet.isLocalIntranet', true, {
        expires: 30,
        domain: '.editorainovacao.com.br',
        secure: true,
        sameSite: 'none'
    ...
    $(img).on('error', function () {
      window.Cookies.remove('intranet.isLocalIntranet', {
        domain: '.editorainovacao.com.br'
      })
      if (isLocalIntranet) {
        window.toastr.info(
          'Conectado a extranet [Home office]',
          editoraInovacao
        )
      }
      isLocalIntranet = false
  ...
  function initialization () {
    ...
    if (window.inlineLoaded || checkIfInlineLoadedFromTop()) {
      console.info(
        'Not initializing inline.js from ' +
          window.self.location +
          ' because it was previously loaded'
      )
      console.groupEnd()
      return
    } else console.info('Initializing inline.js')
    window.inlineLoaded = true
    loadScriptIf(
      typeof $ === 'undefined' ||
        $ == null ||
        $.fn.jquery !== jQueryExpectedVersion,
      'https://code.jquery.com/jquery-' + jQueryExpectedVersion + '.min.js',
      null,
      nc
    )
    loadScriptIf(
      typeof window.toastr === 'undefined',
  ...
  function nc () {
    console.log(
      '$ version: ' +
        (typeof $ !== 'undefined' && $ != null ? $.fn.jquery : 'Not loaded')
    )
    console.log('jQuery version: ' + jQuery.fn.jquery)
    if (jQuery.fn.jquery === jQueryExpectedVersion) return
  ...
  function cookiesLoaded () {
    if (jQuery == null || jQuery.fn.jquery !== jQueryExpectedVersion) {
      ...
      if (jQueryRetries < 30) setTimeout(cookiesLoaded, 1000)
      return
    }
    window.Cookies.json = true
    validateIntranet()
    getPartnerData()
    if (partner === null || partner.UserGroup !== 'Ti-sistema') return
  ...
  function loadIframe (useFrame) {
    ...
    const iframe = $('<' + (useFrame ? '' : 'i') + 'frame />')
    iframe.attr(
      'src',
      'https://intranet.editorainovacao.com.br/postMessage.html'
    )
  ...
  function initializeMessage () {
    window.addEventListener('message', receiveMessage, false)
    const iframe = document.getElementById('inlineJSpostMessage')
    const iframewindow = iframe.contentWindow
      ? iframe.contentWindow
      : iframe.contentDocument.defaultView
    iframewindow.postMessage(
      'Hi from parent',
      'https://intranet.editorainovacao.com.br'
    )
  }

  function receiveMessage (message) {
    console.info(
      'Message received in ' + window.location.href + ' from ' + message.origin
    )
    console.log(message.data)
    if (window.parent !== window.self) {
      window.parent.postMessage(
        'Response to ' + message.data + ' from postMessage.html',
        message.origin
      )
    }
  }
  ...
  function populateInside (inside) {
    if (partner != null && partner.Code > 0) {
      let message = "<div class='personalData'>"
      message +=
        '<b>Ramal:</b> ' +
        partner.UserName +
        ' <b>Grupo: </b> ' +
    ...
    const hesk =
      "<div class='col'><a href='https://help.editorainovacao.com.br'><img src='https://intranet.editorainovacao.com.br/Images/Dials/Hesk.png' alt='HESK - Help Desk' /></a></div>"
    const product =
      "<div class='col'><a href='https://listadecompras.editorainovacao.com.br'><img src='https://intranet.editorainovacao.com.br/Images/Dials/ListaDeCompras.png' alt='Consulta de produtos' /></a></div>"
    const sankhya =
      "<div class='col'><a href='https://sankhya.editorainovacao.com.br'><img src='https://intranet.editorainovacao.com.br/Images/Dials/Sankhya.png' alt='Sankhya-W' /></a></div>"
    const visualizador =
      "<div class='col'><a href='https://visualizador.editorainovacao.com.br'><img src='https://intranet.editorainovacao.com.br/Images/Dials/VisualizadorDePedidos.png' alt='Visualizador de Pedidos' /></a></div>"
    const vendas =
      "<div class='col'><a href='https://vendas.editorainovacao.com.br'><img src='https://intranet.editorainovacao.com.br/Images/Dials/WMWVendas.png' alt='WMW Vendas' /></a></div>"
    const icrm =
      "<div class='col'><a href='https://crm.editorainovacao.com.br'><img src='https://intranet.editorainovacao.com.br/Images/Dials/InovacaoCRM.png' alt='Inovação CRM' /></a></div>"
    const totalip =
      "<div class='col'><a href='https://totalip.editorainovacao.com.br'><img src='https://intranet.editorainovacao.com.br/Images/Dials/TotalIP.png' alt='Total IP' /></a></div>"
    inside.append(hesk)
  ...
  function toggleHelpBar () {
    const helper = $(this).parent()
    const currentState = helper.data('state')
  ...
  function getPartnerByUserId (userId) {
    ...
    getInfoFromIntegracaoAPI(
      'Partners/GetPartnerByUserId?UserId=' + userId,
      'GET',
      null,
      processPartner
    )
  ...
  function getPartnerByCRMName (name) {
    ...
    getInfoFromIntegracaoAPI(
      'Partners/GetPartnerByCRMSellerName?SellerName=' + name,
      'GET',
      null,
      processPartner
    )
  ...
  function getPartnerByExtension (extension) {
    ...
    getInfoFromIntegracaoAPI(
      'Partners/GetPartnerByUserName?UserName=' + extension,
      'GET',
      null,
      processPartner
    )
  ...
  function getPartnerByFingerprint (fingerprint) {
    ...
    getInfoFromVisualizadorAPI(
      'Fingerprint/?fingerprint=' + fingerprint,
      'GET',
      null,
      function (data) {
        if (data.Success) getPartnerByExtension(data.Extension)
      }
    )
  }
  ...
  function processPartner (partner) {
    ...
    console.log(partner)
    window.Cookies.set('intranet.partnerCode', partner.Code, {
      expires: 15,
      domain: '.editorainovacao.com.br',
      secure: true,
      sameSite: 'none'
    ...
    window.Cookies.set('intranet.partner', partner, {
      expires: 5,
      domain: '.editorainovacao.com.br',
      secure: true,
      sameSite: 'none'
  ...
  function getPartner () {
    ...
    if (
      window.location.href.indexOf('totalip') !== -1 &&
      $('#header div.block label').length === 1
    ) {
      return getPartnerByExtension($('#header div.block label').text())
    }
    if (
      window.location.href.indexOf('vendas') !== -1 &&
      $('#nmUserLogado').length === 1
    ) {
      return getPartnerByCRMName($('#nmUserLogado').text())
    }
    if (
      window.location.href.indexOf('visualizador') !== -1 &&
      window.Cookies.get('fingerprint')
    ) {
  ...
  if (
    window.location.hash.indexOf('enable-inlinejs') === -1 &&
