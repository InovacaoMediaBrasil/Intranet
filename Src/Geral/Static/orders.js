...
function processOrdersReport (data) {
  let html =
    '<header>Pedidos do dia ' +
    ToReadableDate(ToJavaScriptDate(data.Date)) +
    " <span class='label label-default'>atualizado às " +
    ToReadableTime(ToJavaScriptDate(data.LastUpdateDate)) +
    '</span></header>'
  html += '<article>'
  html += "<table class='table table-hover table-condensed table-striped'>"
  html +=
    '<thead><tr><th>Operação</th><th>Pagamento</th><th>Pedidos</th><th>Ticket médio</th></tr>'
  html += '<tbody>'
...
function processOrdersGrowth (data) {
  let html =
    "<header>Análise de vendas <span class='label label-default'>atualizado às " +
    ToReadableTime(ToJavaScriptDate(data.LastUpdateDate)) +
    '</span></header>'
  html += '<article>'
  html += "<table class='table table-hover table-condensed table-striped'>"
  html +=
    '<thead><tr><th>Data / Operação</th><th>Todas</th><th>Ativo</th><th>Receptivo</th><th>e-Commerce</th></tr>'
  html += '<tbody>'
  const today = findByProperty(data.Reports, 'Type', 0)
  html += processOrdersGrowthGroup('Hoje', today, null)
  html += processOrdersGrowthGroup(
    'Ontem',
    findByProperty(data.Reports, 'Type', 1),
    today
  )
  html += processOrdersGrowthGroup(
    'Há uma semana',
    findByProperty(data.Reports, 'Type', 2),
    today
  )
  html += processOrdersGrowthGroup(
    'Há um mês',
    findByProperty(data.Reports, 'Type', 3),
    today
  )
  html += processOrdersGrowthGroup(
    'Há um ano',
    findByProperty(data.Reports, 'Type', 4),
    today
  )
  html += '</tbody>'
  html += '</table>'
...
function processOrdersGrowthGroup (group, data, todayData) {
  const all = calculateOrdersGrowth(data, todayData, 0)
  const active = calculateOrdersGrowth(data, todayData, 500)
  const receptive = calculateOrdersGrowth(data, todayData, 501)
  const eCommerce = calculateOrdersGrowth(data, todayData, 502)
  let tr = '<tr>'
  tr += '<td>' + group + '</td>'
  ...
  tr +=
    '<td>' +
    active.Quantity +
    (group !== 'Hoje' ? active.Percent : '') +
    '</td>'
  tr +=
    '<td>' +
    receptive.Quantity +
    (group !== 'Hoje' ? receptive.Percent : '') +
    '</td>'
  tr +=
    '<td>' +
    eCommerce.Quantity +
    (group !== 'Hoje' ? eCommerce.Percent : '') +
    '</td>'
  tr += '</tr>'
  return tr
}
...
function getQuantityOrdersGrowth (group, key) {
  ...
  if (group.Growths === null) return 0
  const item = findByProperty(group.Growths, 'CodeTypeOperation', key)
  ...
  const className =
    quantity === 0 && quantityBase === 0
      ...
      : quantity > quantityBase
        ...
        : 'success'
  let percent =
    ((quantityBase - quantity) * 100) / (quantity === 0 ? 1 : quantity)
  if (quantityBase === 0 && quantity !== 0) percent *= quantity
  return (
    " <br /><span class='label label-" +
    className +
    "' title='" +
    text +
    "' data-toggle='tooltip'>" +
...
function processOrdersWithError (data) {
  ...
  if ($('.ordersPending').length === 1) updateOrdersWithError()
}

function updateOrdersWithError () {
  $('.ordersPending').empty()
  if (
    lastOrdersWithErrorUpdate === null ||
    (pendingOrdersVTEX.length === 0 && pendingOrdersWMW.length === 0)
  ) {
    return
  }
...
function processPendingOrders (systemName, id, pendingOrders) {
  ...
  let html =
    '<header>Erro de importação (' +
    systemName +
    ") <span class='label label-danger'>" +
    pendingOrders.length +
    "</span> <span class='label label-default'>atualizado às " +
    ToReadableTime(ToJavaScriptDate(lastOrdersWithErrorUpdate)) +
    '</span></header>'
  ...
  for (let x = 0; x < pendingOrders.length; x++) {
    ...
    html +=
      ...
      ToReadableDateTime(ToJavaScriptDate(pending.OrderDate))
    html +=
      " | <i class='fa fa-edit' aria-hidden='true'></i> " +
      ToReadableDateTime(ToJavaScriptDate(pending.ChangedDate))
    html += '</div>'
    ...
    html +=
      "<a class='btn btn-xs btn-info' href='https://visualizador.editorainovacao.com.br/?originalOrder=" +
      pending.Code +
      "' target='_blank'>Visualizador de Pedidos</a>"
    html +=
      "<button class='btn btn-xs btn-warning fixOrder' data-originalorder='" +
      pending.Code +
      "' data-systemid='" +
      id +
      "'>Tentar novamente</button>"
    html +=
      "<a class='btn btn-xs btn-success' href='https://help.editorainovacao.com.br/index.php?a=add&category=2&subject=Importação de pedido " +
      systemName +
      ': ' +
      pending.Code +
      "' taget='_blank'>Chamado</a>"
    html += '</div></div></div>'
  }
  html += '</div></article>'
...
function getLastMessageFromHistory (history) {
  if (regexHistory.test(history)) {
    return history.replace(
      regexHistory,
      "<b>$1</b><br /><span style='font-size: 9px;'>$2</span>"
    )
  }
  return history
}
...
function fixOrder () {
  const originalOrder = $(this).data('originalorder')
  let systemName = $(this).data('systemid')
  $.ajax({
    url:
      integracaoEndPointMain +
      'WebHooks/FlagOrderAsFixed?OriginalOrder=' +
      originalOrder +
      '&systemName=' +
      systemName,
    type: 'POST',
    ...
    success: function (response) {
      ...
      if (systemName == 'VTEX') {
        pendingOrdersVTEX = removeByProperty(
          pendingOrdersVTEX,
          'Code',
          originalOrder
        )
      ...
      if ((systemName = 'WMWVendas')) {
        pendingOrdersWMW = removeByProperty(
          pendingOrdersWMW,
          'Code',
          originalOrder
        )
