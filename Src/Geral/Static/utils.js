...
window.onerror = function (errorMsg, url, lineNumber) {
  if (errorMsg.indexOf('Script error') !== -1 && lineNumber === 0) return false
  if (typeof window.toastr !== 'undefined') {
    window.toastr.error(
      errorMsg + ' on line number ' + lineNumber + ' of file ' + url
    )
  }
  return false
}
...
function anyDifferent (array, key, value) {
  for (let x = 0; x < array.length; x++) {
    if (array[x][key] !== value) return true
  }
  return false
}

function anyDifferentFunc (array, func) {
  for (let x = 0; x < array.length; x++) if (func(array[x])) return true
  return false
}
...
function blinker (items) {
  ...
  $(items).each(function () {
    ...
    if (
      !$(this).hasClass('blinking') &&
      (startHours < currentDate.getHours() ||
        (startHours === currentDate.getHours() &&
          startMinutes <= currentDate.getMinutes())) &&
...
function getFingerprint () {
  new Fingerprint2().get(function (result) {
    ...
    if (typeof window.Cookies !== 'undefined' && window.Cookies !== null) {
      window.Cookies.set('fingerprint', result, {
        expires: 365,
      ...
      document.cookie =
        ...
        ';expires=' +
        d.toGMTString() +
        ';domain=.editorainovacao.com.br;Secure;SameSite=none'
    }
  })
}
