function fixCRMService () {
  ...
  $.confirm({
    ...
    content:
      'Você confirma que o <b>WMW Vendas</b> está exibindo a mensagem <b>Já existe um atendimento</b> para todos os usuários ?',
    theme: 'bootstrap',
    icon: 'fa fa-question',
    animation: 'scale',
    type: 'orange',
          ...
          $.ajax({
            ...
            error: function () {
              $.alert({
                title: 'Ocorreu um erro!',
                content:
                  'Por favor, tente novamente ou contate o departamento de TI!',
                theme: 'bootstrap',
...
function fixCRMExtension () {
  ...
  $.confirm({
    ...
    content:
      "<form action='' name='form' class='form-inline'>Informe o ramal que apresenta o problema ao efetuar atendimento no <b>WMW Vendas</b>:" +
...
function getCRMSellerName () {
  const extension = parseInt($('#fixCRMExtensionExtension').val())
  if (isNaN(extension) || extension < 70000 || extension > 79999) return
  if (sellerCodeLoading === extension) return
  sellerCodeLoading = extension
  $.ajax({
    url:
      integracaoEndPointMain +
      'Partners/GetPartnerByUserName?UserName=' +
      extension,
    type: 'POST',
    dataType: 'JSON',
    ...
    success: function (response) {
      const className =
        ...
        response.SellerSupervisorCode > 0
          ...
          : 'danger'
      const sector =
        response.SellerSector.length > 1
          ? response.SellerSector
          : response.UserGroup
      $('#extensionSellerName')
        .html(
          response.SocialName +
        ...
        .css('color', 'black')
        .css('opacity', '1')
      $('#extensionSellerSupervisor')
        .text(response.SellerSupervisor)
        .css('color', 'black')
        .css('opacity', '1')
    ...
    error: function () {
      $.alert({
        ...
        content:
          'Não foi possível localizar o vendedor com o ramal ' + extension,
        theme: 'bootstrap',
        icon: 'fa fa-exclamation-triangle',
        animation: 'scale',
        type: 'red',
