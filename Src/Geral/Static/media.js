...
function onEmptyMediaSchedule (type) {
  let colspan = 5
  if (type === 'week') colspan = 4
  $('#' + type + 'MediaSchedule')
    .empty()
    .append(
      "<tr><td colspan='" +
        colspan +
        "' class='big' id='emptyMediaSchedule'>Não existem mídias agendadas</td></tr>"
    )
}
...
function parseMediaItem (item, type) {
  ...
  let availability = ''
  if (
    typeof item.ProductAvailability !== 'undefined' &&
    !isNaN(item.ProductAvailability)
  ) {
    let className = 'success'
    let orMore = ''
    const s =
      item.ProductAvailability === 1 || item.ProductAvailability === -1
        ? ''
        : 's'
    let title =
      item.ProductAvailability +
      ' ' +
      (s === '' ? 'unidade disponível' : 'unidades disponiveis') +
      ' para venda'
  ...
  if (type === 'today') {
    $('#todayMediaSchedule').append(
      "<tr data-start='" +
        ...
        "'><td>" +
        startTime +
        "</td><td style='text-align:left;padding-left:3px;'><span class='label label-info' data-toggle='tooltip' title='Código do produto: " +
        item.ProductCode +
        "'>#" +
        item.ProductCode +
        '</span> ' +
        availability +
        " <span class='label label-primary' data-toggle='tooltip' title='Professor(a): " +
        item.PartnerName +
        "'>" +
        item.PartnerName +
        "</span> <span class='label label-default' data-toggle='tooltip' title='Mídia: " +
        item.Name +
        "'>" +
        item.Name +
        '</span> <br />' +
    ...
    $('#weekMediaSchedule').append(
      "<tr data-productid='" +
        ...
        "'>#" +
        item.ProductCode +
        "</span> <span class='label label-primary' data-toggle='tooltip' title='Professor(a): " +
        item.PartnerName +
        "'>" +
        item.PartnerName +
        "</span> <span class='label label-default' data-toggle='tooltip' title='Mídia: " +
