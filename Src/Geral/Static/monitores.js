...
$(document).ready(function () {
  ...
  $('#dashboard, #dashboard iframe, .databaseLocks').height(
    $(window).height() + 'px'
  )
  $('#dashboard').slick({
    arrows: false,
...
function getLocks (monitorId) {
  $.ajax({
    url:
      ...
      'Monitor/GetSqlServerLocks?MonitorId=' +
      monitorId,
    type: 'GET',
    dataType: 'JSON',
    success: function (data) {
      if (monitorId === 11) locksSankhya = data
      else if (monitorId === 12) locksWMW = data
...
function updateLocks () {
  const tbody = $('.databaseLocks table tbody')
  tbody.empty()
  if (!alarms.debug) alarms.duration = 0
...
function siren () {
  if (alarms.debug) alarms.duration++
  ...
  if (alarms.duration <= 90) {
    alarms.highlight.removeClass('faster')
    if (alarms.error.paused) alarms.error.play()
    return
  }
  if (alarms.duration <= 120) return
  alarms.highlight.addClass('faster')
  alarms.error.pause()
  if (alarms.danger.paused) alarms.danger.play()
}

function processLock (item, application) {
  const time = item.BlockedDuration / 1000
  if (alarms.duration < time) alarms.duration = time
  const tr = $('<tr></tr>').addClass('danger')
  const thPID = $('<th></th>').text(item.BlockedSessionId)
  tr.append(thPID)
  const tdPID = $('<td></td>').text(item.BlockingSessionId)
  tr.append(tdPID)
  const tdDMS = $('<td></td>').text(time + ' segundos')
  tr.append(tdDMS)
  const tdTYPE = $('<td></td>').text(item.BlockedType)
  tr.append(tdTYPE)
  const tdCMD = $('<td></td>').text(item.BlockingCommand)
  tr.append(tdCMD)
  const tdAPP = $('<td></td>').text(item.BlockingProgram)
  tr.append(tdAPP)
  const tdCLI = $('<td></td>').text(
    item.BlockingUsername + ' / ' + item.BlockingHostname
  )
  tr.append(tdCLI)
  return tr
}
...
function updateMonitorsCards () {
  $('#monitors').empty()
  for (let x = 0; x < monitors.length; x++) populateMonitor(monitors[x])
}
...
function populateMonitor (monitor) {
  ...
  let div =
    ...
    className +
    "' style='16px; height: 60px;border:1px solid #000;padding: 2px;'>"
  div +=
    monitor.Name +
    ' <br /> ' +
    ToReadableTime(ToJavaScriptDate(monitor.LastStatusDate))
  div += '</div>'
  $('#monitors').append(div)
}
...
function updateOrdersCards () {
  if (
    typeof pendingOrdersVTEX === 'undefined' ||
...
function populateOrders (systemName, pendingOrders) {
  const className = pendingOrders.length > 0 ? 'danger' : 'success'
  $('#pendingOrders').append(
    "<div class='col-md-12 bg-" +
      className +
      ' text-' +
      className +
      "' style='font-size: 64px; text-align:center;'><span style='font-size:24px;'>Pedidos com erro de importação (" +
      systemName +
      '):</span> ' +
...
function populateOrder (order) {
  let div = "<div class='col-md-6 panel panel-danger'>"
  div += "<div class='panel-heading'>"
  div += "<i class='fa fa-hashtag' aria-hidden='true'></i> " + order.Code
  div +=
    "<br /><i class='fa fa-clock-o' aria-hidden='true'></i> " +
    ToReadableDateTime(ToJavaScriptDate(order.OrderDate))
  div +=
    "<br /><i class='fa fa-edit' aria-hidden='true'></i> " +
    ToReadableDateTime(ToJavaScriptDate(order.ChangedDate))
  div += '</div>'
  div +=
