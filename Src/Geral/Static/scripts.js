const notificationIconsBasePath =
  'https://intranet.editorainovacao.com.br/Images/Notifications/'
const integracaoEndPointMain = 'https://integracao.editorainovacao.com.br/API/'
const loc = window.location.href
let method = 'GetMediaScheduleForWeek'
const monitors = []
const pendingOrdersVTEX = []
const pendingOrdersWMW = []
const groups = []
let intervalExtensions
const extensionsFilter = null
const extensionsOnlineOnly = true
const extensionsAvailableOnly = true
const sellerCode = 0
const sellerCodeLoading = 0

listaDeCompras_storeId = 1
listaDeCompras_key = 'editorainovacao'
listaDeCompras_token =
  'DCB3C797FAD372E5FF5B95821A872438F588CAA51232BFCDD74C88CAEF'

$(document).ready(initializeIntranet)

function initializeIntranet () {
  if (
    $('#todayMediaSchedule tr').length > 0 &&
    typeof blinker !== 'undefined'
  ) {
    setInterval(function () {
      blinker('#todayMediaSchedule tr')
    }, 1000)
  }
  $("[data-toggle='tooltip']").tooltip()
  if (typeof Fingerprint2 !== 'undefined') setTimeout(getFingerprint, 1000)
  if (typeof updateLinks !== 'undefined') updateLinks()
  if (typeof getMonitors !== 'undefined') getMonitors()
  if (typeof isMonitors !== 'undefined' && isMonitors) return
  updateServiceLinks()
  ...
  if (typeof loadRankingData !== 'undefined') loadRankingData()
  initializeFeeds()
  initializeBanner()
  if ($('.ordersReports').length === 1) initializeOrdersReport()
  if ($('.ordersGrowth').length === 1) initializeOrdersGrowth()
  if ($('.ordersPending').length === 1) initializeOrdersWithError()
}
...
function updateLinks () {
  $.each($('.operation a'), function (index, item) {
    if ($(item).attr('href').indexOf('redirect.php') === -1) {
      $(item).attr(
        'href',
        'https://intranet.editorainovacao.com.br/redirect.php?url=' +
          $(item).attr('href')
      )
    }
  })
...
function buildDials () {
  ...
  for (let x = 0; x < selectedDials.length; x++) {
    const item = findByProperty(dials, 'Name', selectedDials[x])
    if (item == null) continue
    let li = '<li>'
    li += "<a href='" + item.Href + "'>"
    if (item.Monitor) {
      li +=
        "<div class='monitor' data-monitor='" + item.MonitorName + "'></div>"
    }
    li += item.Name
    li += '<br />'
...
function updateServiceLinks () {
  const isLocalIntranet = true & window.Cookies.get('intranet.isLocalIntranet')
  $.each(serviceLinks, function (i, service) {
    const link = isLocalIntranet ? service.urls.local : service.urls.remote
    $(".dials a div.monitor[data-monitor='" + service.service + "']")
      .parent()
      .attr('href', link)
  })
  updateLinks()
